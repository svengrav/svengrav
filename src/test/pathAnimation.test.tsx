...
const genRoutePoints = (start: number, length: number) => Array.from({ length }, (_, i) => i).map(i => ({
  x: i + start,
  y: i + start,
  i: i + start
}))
...
test('calc path 2 parts with pos 1', () => {
  ...
  expect(calcPathPartSize({ parts, points, offset: 1.0 }, 1)).toMatchObject(newParts)
})
...
test('calc path 2 parts with pos 2', () => {
  ...
  expect(calcPathPartSize({ parts, points, offset: 1.0 }, 3)).toMatchObject(newParts)

  // throw if out of range
  expect(() => calcPathPartSize({ parts, points, offset: 1.0 }, 5)).toThrow()
  expect(() => calcPathPartSize({ parts, points, offset: 1.0 }, 0)).toThrow()
})
